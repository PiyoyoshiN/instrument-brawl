...
- [ ] PまたはEscで中央にポーズ画面が表示される
- [ ] ポーズ中は敵・弾・水準時間・一時強化時間・AUTO時間が進まない
- [ ] 操作説明、演奏へ戻る、ランを終了して持ち帰るが表示される
- [ ] 撤退は2回決定しないと実行されない
- [ ] PまたはEscでも安全に再開できる
- [ ] ブラウザをバックグラウンドへ移すと自動でポーズされる

## 選択・結果UI

- [ ] 拠点に4楽器が横並びのカードとして均等配置される
- [ ] 選択中の楽器カードが楽器カラーで強調される
- [ ] 出撃・強化・特別戦が横並びで中央配置される
- [ ] 拠点とリザルトに旧ファイター画像が表示されない
- [ ] リザルト6項目が3列×2段で均等配置される

## 敵解放

- [ ] 楽器Lv.0では、その楽器の敵水準4までが解放される
- [ ] 楽器Lv.2では、その楽器の敵水準6までが解放され、最初のボス水準へ到達できる
- [ ] ギターで水準をクリアしても、未使用のスティック・ベース・キーボードのクリア記録は増えない
- [ ] 使用楽器のクリア済み水準＋1より先から戦闘が始まらない
- [ ] 時間経過で複数水準の敵が重なっても、各水準の敵を全滅させた順にクリア記録へ反映される
- [ ] 解放上限が5の倍数になる場合、原則として次の通常水準まで同時に解放される
- [ ] 楽器Lv.8未満では水準1から始まる
- [ ] 楽器Lv.8以上では開始水準が上がるが、未クリア水準や5の倍数のボス水準から直接始まらない
- [ ] 解放上限到達後は敵水準が上がらず、同水準の限界演奏として継続する
- [ ] 水準3からCharger、水準4からRanged、水準6からBrute、水準8からSupportが混ざる

## ENCORE

- [ ] 通常敵12体分の撃破でENCOREが発動する
- [ ] ボス撃破はENCOREゲージ6体分として扱われる
- [ ] ENCORE発動時に周囲の敵を吹き飛ばす全周攻撃が出る
- [ ] 8秒間、攻撃力・攻撃速度・移動速度が上昇する
- [ ] HUDにENCOREゲージと残り時間が表示される

## 強化画面レイアウト

- [ ] 共通強化6項目が2列×3段で表示される
- [ ] 楽器専用強化が共通強化の下に独立した1段で表示される
- [ ] 最下段の共通強化、専用強化、説明欄が互いに重ならない
- [ ] 強化画面を閉じて2回目・3回目に開いてもフリーズせず、全行を操作できる
- [ ] 前回入場時に破棄された文字オブジェクトが選択更新の対象に残らない

## 自動演奏拡張

- [ ] 自動演奏Lv.0では自動攻撃が発生しない
- [ ] ドラムのバスドラムが一定間隔で前方へ低音衝撃を出す
- [ ] ギターのハーモニーアンプが明るい和音とともに最寄りの敵へ貫通波を放つ
- [ ] ベースのサブウーファーが周囲の敵を押し返す
- [ ] 鍵盤のアルペジエーターが複数の自動音符を放つ
- [ ] 自動演奏Lvを上げると威力が上がり、発動間隔が短くなる
- [ ] 4種類それぞれに異なる専用SEが鳴る
- [ ] HUDに次のAUTO発動までの秒数が表示される

## 野外フェスフィールド・読みやすい配色

- [ ] 通常戦の背景が青空、床が自然色の草地として見える
- [ ] フィールド外周にスピーカー、フェス照明、旗、木、岩、花があり、戦闘中の視界や移動を妨げない
- [ ] Home、拠点、永続強化、リザルトが濃紺・生成り・金縁を基調とし、画面全体が赤くならない
- [ ] 戦闘HUDとポーズ画面が青空・草地の上でも読みやすい
- [ ] ギターのハーモニーアンプSEが不協和な高音やホラー音に聞こえない

## STAGE HEAT

- [ ] 敵撃破でSTAGE HEATが上昇する
- [ ] 撃破が止まって3秒ほど経つとHEATが緩やかに低下する
- [ ] 25 / 50 / 75%でGROOVE / HEADLINER / OVERDRIVEへ変化する
- [ ] HEAT段階に応じて攻撃力、攻撃間隔、移動速度、自動演奏間隔、コイン倍率が上がる
- [ ] 段階上昇時に通知、衝撃波、カメラ揺れが発生する
- [ ] HEAT上昇に合わせてフェス照明と攻撃アクセントが強まり、画面全体は赤くならない
- [ ] ポーズ中はSTAGE HEATが低下しない

## 通常敵キャラクター

- [ ] Walkerが顔・腕・脚・衣装と小型楽器を持つ演奏兵に見える
- [ ] Chargerが角付きヘルメット・肩当て・楽器装備を持つパンク敵に見える
- [ ] Rangedが二連スピーカー・表示部・アンテナを持つドローンに見える
- [ ] Bruteが大型の腕・頭・スピーカー核・背負い楽器を持つゴーレムに見える
- [ ] Supportがフード・音叉・回転する音響輪・楽器を持つ支援役に見える
- [ ] 通常敵の脚や浮遊パーツが移動中に動く
- [ ] 遠距離弾が単なる球ではなく、音圧リングを伴って飛ぶ
...
- [ ] Space、J、クリックを続けて入力すると、左右の薙ぎと全周フィニッシュからなる3段コンボになる
- [ ] 3段目は通常攻撃より広い範囲を巻き込み、強いヒットストップと吹き飛ばしが発生する
- [ ] Kを長押しすると金色のチャージリングとCHORDゲージが伸び、離すと溜めコード攻撃になる
- [ ] 専用Lv.0の溜めコードは強い近接攻撃に留まり、専用Lv.1以上では溜め量に応じて音圧波が増える
- [ ] Shiftで移動入力方向へ回避リフが出て、短い無敵時間中は接触・敵弾ダメージを受けない
- [ ] 回避リフが近くの敵弾を破砕し、連打できないよう再使用待ちがある
- [ ] ギターで敵を倒すとSOLOゲージが増え、100%でREADY表示になる
- [ ] QでGUITAR SOLOを発動すると、全周打撃・12方向音圧・追撃リングで敵集団を吹き飛ばす
- [ ] ポーズ画面と戦闘HUDにギター固有操作が表示される
- [ ] ベース、スティック、キーボードの既存攻撃操作と性能は変わらない
